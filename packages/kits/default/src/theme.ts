...
import { isDarkMode, readReactive } from '@pmndrs/uikit'
...
export type Theme = Omit<typeof slateTheme, 'dark'> & { dark?: Partial<Omit<typeof slateTheme, 'dark'>> }
...
export const theme = signal<Theme>(defaultThemes.slate)
...
for (const anyKey in defaultThemes['slate']) {
  const key = anyKey as keyof (typeof defaultThemes)['slate']
